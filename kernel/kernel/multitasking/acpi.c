...
static const acpi_sdt_header_t* acpi_map_sdt(uintptr_t physical_address)
{
    if (physical_address == 0)
        return NULL;

    /*
     * Map enough memory to read the common ACPI header first.
     */
    ...
    const acpi_sdt_header_t *header =
        (const acpi_sdt_header_t *)physical_address;

    if (header->length < sizeof(acpi_sdt_header_t))
        return NULL;

    /*
     * Now that header->length is known, map the complete table.
     */
    if (!paging_identity_map_range(
            physical_address,
            header->length,
...
const acpi_sdt_header_t* acpi_find_table(const char signature[4])
{
     ...
     */
    const acpi_sdt_header_t *rsdt = acpi_map_sdt((uintptr_t)root_rsdp->rsdt_address);
    ...
    {
        const acpi_sdt_header_t *table = acpi_map_sdt((uintptr_t)entries[i]);
