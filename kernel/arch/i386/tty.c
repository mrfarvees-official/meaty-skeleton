...
static terminal_cell_t
	framebuffer_cells[FB_TERMINAL_MAX_ROWS]
...
static uint32_t terminal_foreground_rgb(
	uint8_t color)
{
	return terminal_rgb_palette[color & 0x0Fu];
}

static uint32_t terminal_background_rgb(
	uint8_t color)
{
	return terminal_rgb_palette[(color >> 4) &
								0x0Fu];
}
...
static void terminal_framebuffer_cursor_show(void)
{
	...
	terminal_cell_t cell =
		framebuffer_cells[terminal_row]
						 [terminal_column];
...
static void terminal_framebuffer_scroll(void)
{
		 ...
		 ++row)
	{
		for (size_t column = 0u;
			 ...
			 ++column)
		{
			framebuffer_cells[row - 1u]
							 [column] =
								 framebuffer_cells[row]
												  [column];
		}
	}
...
static void terminal_vga_scroll(void)
{
		 ...
		 ++row)
	{
			 ...
			 ++column)
		{
			...
			terminal_vga_buffer[destination] =
				terminal_vga_buffer[source];
		}
	}
...
{
	extern void
	mouse_cursor_begin_framebuffer_update(void);

	extern void
	mouse_cursor_end_framebuffer_update(void);

	bool framebuffer_update =
		terminal_backend ==
		TERMINAL_BACKEND_FRAMEBUFFER;
	...
	{
		/*
		 * Hide the software mouse cursor and suspend cursor redraw
		 * while this logical terminal update changes framebuffer
		 * contents.
		 *
		 * This is separate from the terminal's own underline cursor.
		 */
	...
	{
		/*
		 * Finish the framebuffer terminal update first, including
		 * its existing underline cursor.
		 *
		 * The mouse cursor then captures these final pixels as its
		 * new backing store.
		 */
		terminal_framebuffer_cursor_show();
